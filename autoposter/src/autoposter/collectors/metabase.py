...
__all__ = ["collect_metabase", "collect_metabase_multi"]
...
def collect_metabase(
    ...
    return ToolRunStats(tlc_runs=tlc_runs)
...
def _extract_multi_month_counts(
    payload: dict[str, Any],
    months: list[tuple[int, int]],
) -> dict[tuple[int, int], int]:
    """Extract counts for multiple (year, month) pairs from a single response."""
    data = payload.get("data")
    if not isinstance(data, dict):
        return {}
    ...
    targets = {f"{y:04d}-{m:02d}": (y, m) for y, m in months}
    result: dict[tuple[int, int], int] = {}

    for row in rows:
        if date_idx is None:
            continue
        val = str(row[date_idx]).strip()[:7]  # "YYYY-MM"
        if val in targets:
            try:
                result[targets[val]] = int(row[count_idx])
            except (TypeError, ValueError, IndexError):
                pass
...
def collect_metabase_multi(
    dashboard_url: str,
    months: list[tuple[int, int]],
) -> dict[tuple[int, int], int]:
    """Fetch TLC run counts for multiple months in a single API call.
    ...
    dashboard_url:
        Full URL of the public Metabase dashboard.
    months:
        List of ``(year, month)`` tuples to extract.

    Returns
    -------
    dict
        Mapping of ``(year, month)`` to TLC run count. Missing months
    ...
    target = _find_target_dashcard(dashboard.get("dashcards", []))
    if target is None:
        return {}
    ...
    payload = _query_card(dash_uuid, dashcard_id, card_id)
    if payload is None:
        default_params = _build_default_params(dashboard.get("parameters", []))
        payload = _query_card(dash_uuid, dashcard_id, card_id, default_params)
    if payload is None:
        return {}

    return _extract_multi_month_counts(payload, months)
