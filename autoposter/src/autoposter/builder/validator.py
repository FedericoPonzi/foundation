...
def _check_frontmatter(content: str, errors: list[str]) -> None:
    """Validate Hugo TOML frontmatter structure."""
    if not content.startswith("+++"):
        errors.append("Post does not start with Hugo TOML frontmatter ('+++')")
        return

    parts = content.split("+++", 2)
    # parts[0] is empty (before the first +++), parts[1] is the TOML block
    if len(parts) < 3:
        errors.append("TOML frontmatter is not properly closed with a second '+++'")
        return

    fm_text = parts[1].strip()
    if "title" not in fm_text:
        errors.append("Frontmatter is missing required key 'title'")
    if "type" not in fm_text:
        errors.append("Frontmatter is missing required key 'type'")
